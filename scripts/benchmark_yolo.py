"""
benchmark_yolo.py — Phase 3: YOLOv8n ONNX CPU inference benchmark

Runs YOLOv8n (ONNX) on 100 random VisDrone images and records latency/FPS.

Usage:
    python scripts/benchmark_yolo.py
"""

import json
import os
import random
import sys
import time
from pathlib import Path

import numpy as np
import onnxruntime as ort
from PIL import Image

ROOT      = Path(__file__).resolve().parent.parent
IMG_DIR   = ROOT / "data" / "VisDrone2019-DET-train" / "images"
ONNX_PATH = ROOT / "yolov8n.onnx"
OUT_JSON  = ROOT / "data" / "results_yolo.json"

N_IMAGES  = 100
WARMUP    = 3
SEED      = 42


def preprocess(img_path: Path) -> np.ndarray:
    """Load and preprocess one image to [1, 3, 640, 640] float32 in [0, 1]."""
    img = Image.open(img_path).convert("RGB").resize((640, 640), Image.LANCZOS)
    arr = np.array(img, dtype=np.float32) / 255.0   # [640, 640, 3]
    arr = arr.transpose(2, 0, 1)                     # [3, 640, 640]
    return np.ascontiguousarray(arr[np.newaxis, :])  # [1, 3, 640, 640]


def percentile(values: list[float], p: float) -> float:
    sorted_v = sorted(values)
    idx = (len(sorted_v) - 1) * p / 100.0
    lo, hi = int(idx), min(int(idx) + 1, len(sorted_v) - 1)
    return sorted_v[lo] + (sorted_v[hi] - sorted_v[lo]) * (idx - lo)


def main() -> None:
    if not ONNX_PATH.exists():
        print(f"ERROR: {ONNX_PATH} not found. Run export first.", file=sys.stderr)
        sys.exit(1)

    images = sorted(IMG_DIR.glob("*.jpg"))
    if len(images) < N_IMAGES:
        print(f"ERROR: only {len(images)} images available.", file=sys.stderr)
        sys.exit(1)

    rng = random.Random(SEED)
    sample = rng.sample(images, N_IMAGES)

    print(f"Loading {ONNX_PATH.name} ({os.path.getsize(ONNX_PATH)/1e6:.2f} MB) ...")
    session = ort.InferenceSession(
        str(ONNX_PATH),
        providers=["CPUExecutionProvider"],
    )
    input_name = session.get_inputs()[0].name

    # Warmup
    warmup_tensor = preprocess(sample[0])
    print(f"Warming up ({WARMUP} iters) ...")
    for _ in range(WARMUP):
        session.run(None, {input_name: warmup_tensor})

    # Benchmark
    latencies_ms: list[float] = []
    print(f"Benchmarking {N_IMAGES} images ...")
    for i, img_path in enumerate(sample, start=1):
        tensor = preprocess(img_path)
        t0 = time.perf_counter()
        session.run(None, {input_name: tensor})
        t1 = time.perf_counter()
        latencies_ms.append((t1 - t0) * 1000.0)
        if i % 25 == 0:
            print(f"  [{i}/{N_IMAGES}]  last={latencies_ms[-1]:.1f} ms")

    mean_ms = float(np.mean(latencies_ms))
    std_ms  = float(np.std(latencies_ms))
    p50     = percentile(latencies_ms, 50)
    p95     = percentile(latencies_ms, 95)
    fps     = 1000.0 / mean_ms

    weight_bytes = os.path.getsize(ONNX_PATH)

    result = {
        "model": "yolov8n-onnx",
        "n_images": N_IMAGES,
        "weight_size_bytes": weight_bytes,
        "latency_ms": {
            "mean": round(mean_ms, 3),
            "std":  round(std_ms,  3),
            "p50":  round(p50,     3),
            "p95":  round(p95,     3),
            "min":  round(min(latencies_ms), 3),
            "max":  round(max(latencies_ms), 3),
        },
        "fps": round(fps, 2),
    }

    OUT_JSON.parent.mkdir(parents=True, exist_ok=True)
    with open(OUT_JSON, "w") as f:
        json.dump(result, f, indent=2)

    print(f"\n{'='*42}")
    print(f"  Model        : {result['model']}")
    print(f"  Weight size  : {weight_bytes/1e6:.2f} MB")
    print(f"  Mean latency : {mean_ms:.1f} ms  (±{std_ms:.1f})")
    print(f"  p50 / p95    : {p50:.1f} / {p95:.1f} ms")
    print(f"  FPS          : {fps:.1f}")
    print(f"{'='*42}")
    print(f"Results saved to {OUT_JSON}")


if __name__ == "__main__":
    main()
